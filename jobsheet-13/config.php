<?php
    class Database {
        ...
        function tambah_data_peminjam($kode_peminjaman, $nama_peminjam, $jenis_kelamin, $tanggal_lahir, $alamat, $pekerjaan, $foto) {
            mysqli_query($this->koneksi, "INSERT INTO data_peminjam VALUES('', '$kode_peminjaman', '$nama_peminjam', '$jenis_kelamin', '$tanggal_lahir', '$alamat', '$pekerjaan', '', '$foto')");
        ...
        function edit_data_peminjam($kode_peminjam, $nama_peminjam, $jenis_kelamin, $tanggal_lahir, $alamat, $pekerjaan, $foto) {
            mysqli_query($this->koneksi, "UPDATE data_peminjam SET nama_peminjam='$nama_peminjam', jenis_kelamin='$jenis_kelamin', tanggal_lahir='$tanggal_lahir', alamat='$alamat', pekerjaan='$pekerjaan', foto='$foto' WHERE kode_peminjam='$kode_peminjam'");
        }
        ...
        function tampil_data_peminjaman_peminjam($username){
            $data = mysqli_query($this->koneksi, "SELECT a.*, b.*, c.* FROM peminjaman a INNER JOIN data_buku b ON b.kode_buku = a.kode_buku INNER JOIN data_peminjam c ON c.kode_peminjam = a.kode_peminjam WHERE a.kode_peminjam='$username'");
            while($row = mysqli_fetch_array($data)) {
                $hasil[] = $row;
            }
            return $hasil;
